...
class GAN:
	...
	def build_generator(self):
		...
		t_real_caption = tf.placeholder('float32', [self.options['batch_size'], self.options['caption_vector_length']], name = 'real_caption_input')
		t_z = tf.placeholder('float32', [self.options['batch_size'], self.options['z_dim']])
		fake_image = self.generator(t_z, t_real_caption,training=False)
	...
	def generator(self, t_z, t_text_embedding,training=True,name='gan-generator'):
	...
	def discriminator(self, image, t_text_embedding, reuse=False,name='gan-discriminator'):
		with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
